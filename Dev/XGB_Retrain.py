...
#         
# Retrains models on entire balanced dataset with optimised hyperparameters for final testing
...
# %%
def BF_fitting(BF, d_train_list, final_param): 
    """ 
    Input:      BF                Number of balancing folds                      
                d_train_list      List of balanced training feature folds in DMatrix
                d_val             Validation data as Dmatrix
    ...
    for fold_i in range(BF):
        ...
        model = xgb.train(params,                                   #Generates and fits a GBC for each training balanced fold
                          d_train, 
                          num_boost_round = int(final_param[0]['num_boost_round']),
                          )
...
# %%
def final_model_train(BF, d_train_list, pickle_file, final_params): 
    """ 
    Input:      BF                Number of balancing folds                      
                d_train_list      List of balanced training feature folds in DMatrix
                pickle_file       List of models to save
                final_params      Optimised hyperparmeters from best fold 
    ...
    for fold_i in range(BF):
        ...
        model = xgb.train(params,                                   #Generates and fits a GBC for each training balanced fold
                          d_train, 
                          num_boost_round = int(final_params['num_boost_round']),
                          )
        final_BF_GBC.append(model)
        
        filename = f"final_model_{fold_i + 1}.pkll"
...
for fold in range(len(IT_list)):          
    ...
    BF_GBC                      = BF_fitting(BF, d_train_list, final_param)
...
final_BF_GBC                = final_model_train(BF, d_train_list, pickle_file, final_params)

print("Testing...") 
final_prob_matrix                 = BF_predict(final_BF_GBC, d_test)
final_final_vote                  = Weighted_Vote(final_prob_matrix)
final_MCC                         = CV_evaluation(d_test, final_final_vote)
